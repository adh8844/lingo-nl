...
async function applyInactivityPenalty(supabase: any, player_id: string, player: any, today: string) {
  const pts: { points: number; reason: string }[] = []
  
  const lastPlayed = player.last_played_date
  if (!lastPlayed) return pts
  
  // Calculate days between last_played_date and today
  ...
  if (diffDays <= 1) return pts // No penalty if played yesterday or today
  
  // Check if we already applied penalty for these days
  const { data: existingPenalties } = await supabase
    .from('points_log')
    .select('reason')
    .eq('player_id', player_id)
    .eq('reason', `Inactiviteitsboete`)
    .gte('created_at', `${today}T00:00:00+00:00`)
    .lte('created_at', `${today}T23:59:59+00:00`)
    .limit(1)
  
  if (existingPenalties && existingPenalties.length > 0) return pts
  
  // For each missed day (excluding today and last_played_date), check if games were played
  ...
  if (missedDays.length === 0) return pts
  
  // Check which of those days had games
  const { data: completions } = await supabase
    .from('game_completions')
    .select('completed_date')
    .eq('player_id', player_id)
    .in('completed_date', missedDays)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Apply inactivity penalty BEFORE processing this game
    const penaltyPts = await applyInactivityPenalty(supabase, player_id, player, today)
    pts.push(...penaltyPts)
    ...
    // 9. Badge checking (skip for challenger games)
    ...
    if (![10, 12, 14].includes(level)) {
      ...
      // Tijd badges
      // Helper: get hour in CET/CEST (Europe/Amsterdam) using Intl.DateTimeFormat
      const getCETHour = (dateStr: string): number => {
        const d = new Date(dateStr)
        const formatter = new Intl.DateTimeFormat('en-GB', { timeZone: 'Europe/Amsterdam', hour: 'numeric', hour12: false })
      ...
      if (!earnedIds.has('vroege_vogel') && games.filter((g: any) => { const h = getCETHour(g.played_at); return h >= 5 && h < 7 }).length >= 5) tryAward('vroege_vogel')
      if (!earnedIds.has('maneschijn') && games.filter((g: any) => { const h = getCETHour(g.played_at); return h >= 22 }).length >= 2) tryAward('maneschijn')
      ...
      if (!earnedIds.has('marathonloper')) {
        ...
        games.forEach((g: any) => { const k = new Date(g.played_at).toISOString().split('T')[0]; dc[k] = (dc[k] || 0) + 1 })
        if (Object.values(dc).some(c => c >= 10)) tryAward('marathonloper')
      }
      if (!earnedIds.has('golfrijder') && session_id && games.filter((g: any) => g.session_id === session_id).length >= 5) tryAward('golfrijder')
    ...
    // Insert points (filter out zero-point entries)
    const nonZeroPts = pts.filter(e => e.points !== 0)
    const totalEarned = pts.reduce((s, e) => s + e.points, 0)
    if (nonZeroPts.length > 0) {
      await supabase.from('points_log').insert(nonZeroPts.map(e => ({ player_id, points: e.points, reason: e.reason, game_id: game.id })))
    }
    ...
    if (!unlocked5) {
      ...
      if (!unlocked5) {
        const { data: allG } = await supabase.from('games').select('id').eq('player_id', player_id).eq('solved', true).eq('attempts', 1)
        if ((allG || []).length >= 5) unlocked5 = true
      }
    }
    ...
    return new Response(JSON.stringify({
      ...
      unlocked_5letter: unlocked5,
      unlocked_6letter: unlocked6,
      trigger_challenger: shouldTriggerChallenger,
