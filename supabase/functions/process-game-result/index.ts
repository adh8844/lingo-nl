...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: ownerCheck } = await supabase
      .from('players').select('user_id').eq('id', player_id).single()
    if (!ownerCheck || ownerCheck.user_id !== authUserId) {
    ...
    if (![10, 12, 14].includes(level)) {
      ...
      // Tijd badges
      // Use Europe/Amsterdam consistently and only award a time badge when the current game
      // itself was played inside that badge's local time window.
      const getAmsterdamHour = (dateStr: string): number => {
